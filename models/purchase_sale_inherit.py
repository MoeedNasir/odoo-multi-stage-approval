...
class PurchaseOrder(models.Model):
    ...
    def action_approve(self):
        ...
        for order in self:
            ...
            next_stage = order._get_next_stage()
            if next_stage:
    ...
    def _get_next_stage(self):
        ...
        self.ensure_one()
        stages = self.approval_flow_id.stage_ids.sorted('sequence')
        current_index = stages.ids.index(self.approval_stage_id.id)
...
class SaleOrder(models.Model):
    ...
    def action_request_approval(self):
        """Sales order specific approval request"""
        for order in self:
            ...
            # Similar logic to purchase order approval
            flow = order.approval_flow_id or self.env['approval.flow'].search([
                ('model', '=', 'sale.order'),
                ('company_id', '=', order.company_id.id)
            ], limit=1)
    ...
    # Override sale order confirmation to require approval
